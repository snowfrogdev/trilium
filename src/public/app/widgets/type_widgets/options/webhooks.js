...
export default class WebhooksOptions extends OptionsWidget {
  doRender() {
    this.$widget = $(TPL);
          ...
          .append(
            $("<td>").append(
              $(
                ...
                this.renameToken(token.etapiTokenId, token.name)
              ),
              $(
                '<span class="bx bx-trash token-table-button" title="Delete / deactivate this token"></span>'
              ).on("click", () =>
                this.deleteToken(token.etapiTokenId, token.name)
              )
            )
          )
      );
  ...
  async deleteToken(etapiTokenId, name) {
    if (
      !(await dialogService.confirm(
        `Are you sure you want to delete ETAPI token "${name}"?`
      ))
    ) {
      return;
    }
